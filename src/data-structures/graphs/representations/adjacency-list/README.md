...
`adjacency_list.AdjacencyList` must be importable. The test scaffold names `create(directed)`, `add_node(value)`, stable dense indexes, handle-based weighted edges, and GraphView index adaptation.
...
Run `python -m pytest src/data-structures/graphs/representations/adjacency-list`. The import check must pass; the behavior scaffold is intentionally marked xfail until concrete tests are written.
